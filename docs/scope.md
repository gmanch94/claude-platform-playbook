...
8 artifacts. ~2 sessions to ship cleanly. (The repo now ships 51 — see the post-v1 justification blocks below.)
...
**Why it earns its place.** `reference-architectures.html` has one agentic workflow pattern diagram. The feature-inventory lists sub-agents as GA. But neither artifact addresses the question architects face when designing agentic systems: *which decomposition pattern fits, and what are the failure modes?* The `claude-misconceptions.md` addition ("multi-agent ≠ more reliable") names the error-compounding problem but offers no scaffolding. This artifact provides 6 named patterns (orchestrator-worker, parallel fan-out, sequential pipeline, validator-retry, human-in-loop gate, dynamic workflows), the compounding accuracy math, sub-agent configuration shape, and the decision criteria for when to use each.
...
### `user-mindset-cheatsheet-color.html` (added 2026-06)

**Why it earns its place.** A user asked for the cheatsheet to be colour-coded ("it looks too boring"). Markdown can't carry colour on GitHub — github.com strips inline styles and there's no native coloured text — so the durable way to deliver it is HTML. This is the **colour-coded / print form** of [`user-mindset-cheatsheet.md`](../artifacts/user-mindset-cheatsheet.md): green = the shift to make, amber = the trap, red = the data boundary, orange = Start-here, reusing the [`user-mindset-mindmap.html`](../artifacts/user-mindset-mindmap.html) palette so the three forms read as one visual set. Same precedent as the mindmap and `enterprise-data-boundaries.html` — a visual rendering earns a slot when the prose form can't do the visual job.
...
### `enterprise-deployment-guide.md` (added 2026-07)

**Why it earns its place.** [`token-budget-governance.md`](../artifacts/token-budget-governance.md) names the prod/non-prod split only at the budget layer ("cap experiments, alert prod" + the org → workspace → workload ladder); [`enterprise-data-boundaries.html`](../artifacts/enterprise-data-boundaries.html) draws per-*feature* trust zones, not per-*environment* ones; [`procurement-pack.md`](../artifacts/procurement-pack.md) §4 covers the hyperscaler *paths* but not how to lay prod vs non-prod across them. The isolation/environment/workspace primitives exist across five artifacts; none assembles them into the deployment-topology decision an enterprise architect actually makes — tenancy (shared-SaaS reality vs the logical-isolation levers) and environment separation (prod / staging / dev via Console workspaces, with the gate between them). Empirically surfaced by two consecutive reader questions — "single- vs multi-tenant Claude Enterprise," then "prod/non-prod setup" — both routing to nothing. A gap named twice by a real reader is the repo's inclusion trigger.

**Why it's not in the original 8.** Original scope treated deployment topology as implicit — the API "just works," seats "just get provisioned." The enterprise-architecture layer (which workspaces, which keys, which model pins, which governance posture per environment, and what gates promotion) is a distinct decision that only surfaces at real enterprise scale. It sits alongside `operating-model-guide.md` (who owns Claude) and `procurement-pack.md` (how you buy it) as the third "enterprise setup" decision — how you lay it out.

**Posture.** Decision-frame first, router-not-pile (every section routes into an existing control — token-budget for caps, eval-pack for the promotion gate, deprecation-runbook for staged cutover, governance-overlay + enterprise-data-boundaries for per-environment posture, procurement §4 for the hyperscaler mapping). Two honesty guards: (1) the single-tenant/dedicated-deployment question is stated as **verify-with-Anthropic, never asserted** — the commercial platform is multi-tenant SaaS with logical-isolation levers, and whether a dedicated deployment exists for very large enterprises is an at-signing question; (2) Console workspace cap/ZDR/BAA granularity carries the same verify-in-Console caveat the token-budget guide already uses. The load-bearing rule — **non-prod must not carry real regulated data unless it holds prod's full ZDR/BAA/residency posture** — is the guide's own top failure mode. Pinned to current surface; refreshed monthly.

### `claude-enterprise-architecture.html` (added 2026-07)

**Why it earns its place.** No artifact shows the **whole** Claude Enterprise feature surface in one view. [`docs/feature-inventory.md`](feature-inventory.md) is the source-of-truth *table* but not a navigable at-a-glance map; [`reference-architectures.html`](../artifacts/reference-architectures.html) draws 6 solution *patterns*, not the full catalog; [`feature-decision-matrix.html`](../artifacts/feature-decision-matrix.html) is a patterns×features *grid*; [`enterprise-data-boundaries.html`](../artifacts/enterprise-data-boundaries.html) is per-feature *trust zones*. An exec or architect asking "show me everything Claude Enterprise can actually do, organized" — the orientation/scoping question upstream of every decision tool — routed to a table. This is the visual full-surface map: a layered stack (access → human surfaces → developer surfaces → capabilities → models → deployment paths) with a cross-cutting governance spine, every box status-tagged (GA/beta/preview) and routing to the decision artifact for that choice. User-requested.

**Why it's not in the original 8.** Original scope treated `reference-architectures.html` (6 patterns) as the visual and assumed the feature-inventory table answered "what exists." Empirically a table isn't a surface map — onboarding a leader to the *breadth* of the platform, and giving an architect a single sheet that indexes every surface + capability + governance control, needs the layered visual. It sits beside the two existing visual companions (`user-mindset-mindmap.html`, `enterprise-data-boundaries.html`) as a third: a visual rendering that does work the prose/table can't.

**Posture.** **Structured layered style, a deliberate deviation from the hand-drawn house style** — a ~45-feature catalog is illegible in Caveat/Patrick-Hand hand-drawn geometry, so the hand-drawn style stays scoped to `reference-architectures.html` + `enterprise-data-boundaries.html` and this uses a clean tiered HTML/CSS layout (user chose structured over hand-drawn). Strictly grounded in `feature-inventory.md` — every box is an inventory row, status tags mirror inventory status, and it makes **no new feature claims**. Governance held to the repo's discipline: BAA stated as admin-activated + per-surface-verify (never asserted covered), Cowork/Design marked BAA-excluded, Tag/Science marked governance-unstated, Fable-5-not-ZDR and hyperscaler-residency caveats carried, "not legal advice." Because it's a full-surface *status* map, a stale tag is a factual error — so the inventory refresh checklist now names it as must-re-check on any status flip. Independent review before ship. Every box cross-links the decision artifact for that choice (deployment-guide, model-selection, feature-decision-matrix, subscription-selection, reference-architectures).

### `claude-code-101.md` + `claude-code-enterprise-config.md` (added 2026-07, ship as a pair)

**Why they earn their place.** The repo's Claude Code cluster had three altitudes — org rollout ([`claude-code-adoption-guide.md`](../artifacts/claude-code-adoption-guide.md)), team building-blocks (the [`hooks`](../artifacts/hooks-starter-pack.md)/[`mcp`](../artifacts/mcp-starter-pack.md)/[`skills`](../artifacts/claude-code-starter-skills.md) packs), and non-engineer mindset ([`user-mindset-cheatsheet.md`](../artifacts/user-mindset-cheatsheet.md)) — but two slots were empty. **`claude-code-101.md`** is the individual-engineer fluency layer: the practitioner who just installed the CLI and wants to drive it well. It's the engineer analog of the user-mindset cheatsheet (which serves non-engineers) — curated *judgment* (which command / permission mode / model alias to reach for and why, each with its gotcha), not a docs mirror. **`claude-code-enterprise-config.md`** is the admin-config layer: the org owner standardizing Claude Code needs three deployable templates (reference organization instructions, `managed-settings.json`, enterprise `CLAUDE.md`) and the one distinction that governs them — managed settings **enforce**, managed `CLAUDE.md` only **guides**. Both were user-requested in the same session.

**Why they're not in the original 8.** Original scope treated Claude Code as a single org-rollout decision (the adoption guide) and assumed individual fluency was "read the docs" and admin config was "set some settings." Empirically each is a distinct decision layer. The 101's value is the judgment *over* the docs — the gotchas the reference pages list but don't rank (`ANTHROPIC_API_KEY` silently overriding a subscription, `@`-imports not saving context, `opusplan` being the better default nobody uses). The enterprise config's value is the enforce-vs-guide mechanics — managed-settings precedence, `forceLoginOrgUUID` org lock, deny-beats-allow, the five managed-delivery sources where only one wins — a decision that only surfaces at org scale and routed to nothing. They are the fluency and config companions to the adoption guide, the way the hooks/mcp packs are its building-block companions.

**Posture.** **Curated, not transcribed** (an explicit advisor discipline — a public guide that mirrors the CLI reference goes stale in weeks and duplicates canonical docs). The 101 is ~17 judgment-first sections each naming a failure mode; the enterprise file is the enforce-vs-guide spine + three org-neutral templates, every `managed-settings.json` field carrying its failure-mode-if-omitted (house tone). Both **reference `feature-inventory.md` for the model surface rather than restating pricing** (no third copy to drift), and link volatile version-pinned specifics to `docs.claude.com` instead of pinning them inline. All Claude Code mechanics verified **[H]** against `docs.claude.com/en/docs/claude-code` 2026-07-11 — including the verbatim `managed-settings.json` paths (macOS `/Library/Application Support/ClaudeCode/`, Linux `/etc/claude-code/`, Windows `C:\Program Files\ClaudeCode\`; legacy `ProgramData` removed v2.1.75) and the five-source managed-delivery precedence. The enterprise file frames delivery as a **choice among four mechanisms**, not "drop the file here," to avoid misconfiguring an MDM/server-managed fleet; it does **not** assert BAA/ZDR/residency coverage (routes to [`governance-overlay.md`](../artifacts/governance-overlay.md)). The 101 is deliberately **off the executive-decision charter** (practitioner how-to) — justified as the named-but-empty engineer-fluency slot in an existing cluster, the same way `user-mindset-cheatsheet.md` cleared the bar as scope-completion for a named-but-unsupplied tier. Both reviewed by two independent blind lenses (accuracy/sourcing + audience/consistency) before ship. Cross-linked to each other and to adoption-guide, starter-skills/hooks/mcp packs, model-selection, operating-model, enterprise-deployment, governance-overlay.

### `claude-product-101.md` (added 2026-07)

**Why it earns its place.** The repo built a mechanics-and-judgment fluency guide for one surface — [`claude-code-101.md`](../artifacts/claude-code-101.md), for the engineer in the CLI — and never built its twin for the surfaces most seats live on (chat, Projects, Cowork, Tag). The obvious objection is [`user-mindset-cheatsheet.md`](../artifacts/user-mindset-cheatsheet.md), and it must be met head-on: the cheatsheet already owns the **behavior-change** layer (its seven shifts), a **"Best surface" table** mapping task → Chat/Projects/Cowork, and an **"Always verify these"** list. So the surface-choice and verify-discipline slots are **filled**. What the cheatsheet deliberately does not carry — it says so: *"about mindset, not prompt mechanics"* — is the **mechanics layer**: how Project knowledge retrieval and staleness and share-scope actually behave, when extended thinking earns its latency, what attachments and web search will and won't do, what Artifacts are for, how Tag threads see context and under whose identity, what a connector exposes. That layer exists for the engineer (`claude-code-101.md`) and for no one else. This guide is that missing half — the product-surface mechanics, curated as judgment (which feature to reach for and the gotcha on each), not a help-center mirror.

**Why it's not in the original 8.** Original scope assumed product mechanics were self-evident from the UI and that end-user enablement was training material, not a decision tool. The Code 101 disproved that for the CLI — the value was ranking the non-obvious defaults (what `opusplan` costs you, why `@`-imports don't save context) — and the same non-obvious-mechanics gap exists on the product surface (what Project knowledge does when it's stale, when extended thinking is wasted latency, what web search won't cite, whose identity a Slack tag runs under). The cheatsheet closed the mindset half of the practitioner slot; this closes the mechanics half. It rides the same bar `claude-code-101.md` cleared: the named-but-empty individual-fluency slot in an existing cluster (scope-completion for a practitioner tier the repo already serves on the engineering side).

**Posture.** **Curated, not transcribed** — the Code 101's advisor discipline: a guide that mirrors the product help-center goes stale in weeks. Judgment-first, ~11 sections, every one naming a failure mode (house tone) — including an explicit-consent / autonomy-hard-stop section (§9: on action surfaces, a general instruction is consent to the *task*, not to every irreversible step; the ask-first vs hard-stop tiers; the "can this be undone quietly?" test). **Center of gravity is the mechanics layer** (Projects/knowledge behavior, extended thinking, attachments, web search, Artifacts, Tag identity/visibility, connectors); surface-choice, the get-good reflexes, and the verify-list are **explicit pointers into [`user-mindset-cheatsheet.md`](../artifacts/user-mindset-cheatsheet.md), not re-derivations** (the cross-file-consistency rule — the cheatsheet is canonical for those, so no second copy drifts). **References `feature-inventory.md` for surface/model facts** (no third copy to drift); volatile UI mechanics (exact size limits, current toggles) link to `support.claude.com` / `docs.claude.com` rather than being pinned inline. **Asserts no BAA/ZDR/residency coverage** — states the one user-facing boundary (confirm your surface is cleared; no PHI in Cowork/Design) and routes to [`governance-overlay.md`](../artifacts/governance-overlay.md) + [`enterprise-data-boundaries.html`](../artifacts/enterprise-data-boundaries.html). Off the executive-decision charter (practitioner how-to), justified like `user-mindset-cheatsheet.md` and `claude-code-101.md` — a named practitioner tier the repo serves unevenly. Every mechanics claim traces to a `feature-inventory.md` row (each with its `[H]` doc anchor); behavioral tendencies (RAG-retrieval feel, staleness) are labeled as such, not asserted as spec. Named `claude-product-101.md` **not** `claude-enterprise-101.md` — the three `enterprise-*` artifacts (`enterprise-config`, `enterprise-architecture`, `enterprise-data-boundaries`) are all admin/architecture, and a *practitioner* guide in that namespace would read as admin. Cross-linked bidirectionally to the cheatsheet (mindset + surface-choice canonical there), the Cowork guide (surface depth), the Code 101 (engineer twin), surface-rollout (what's cleared), model-selection (tier choice), governance-overlay + enterprise-data-boundaries (the boundary). Pinned to current surface; refreshed monthly.

### `index.html` — visual Pages landing page (added 2026-07)

**Why it earns its place.** The repo has three front doors and until now the *default* one was the weakest for a first-time visitor. [`decision-spine.html`](../artifacts/decision-spine.html) is the **guided** route (walk seven decisions in order); the [`README.md`](../README.md) is the **reference index** (30+ row role map + full 51-row catalog). But GitHub Pages serves the README at the site root, so the landing experience was a wall of markdown tables — comprehensive, not scannable, no filtering. A CIO who lands via a forwarded link scans for ~5–10 seconds; the README makes them build the map themselves. `index.html` is the **visual router**: a root landing page that lays the artifacts out as the journey an org actually walks (Evaluate → Decide → Pilot → Scale → Operate), with a primary CTA into the spine, a small featured shelf, an orient strip, and client-side persona / type / search filters. It answers the literal 5-second question ("where do I start?") that the table can't. Two independent experts (a UI/IA lens and a domain/IA lens) were briefed blind and converged on this shape; the one fork they split on — decision-stage spine vs abstraction-layer grid — was decided by the user in favor of the decision-stage journey.

**Why it's not in the original 8 (or the prior additions).** Like `decision-spine.html`, this is **not new content** — every card routes to an existing artifact; it's a new **navigation primitive**. The spine made the artifacts addressable as a guided flow; this makes them addressable as a filterable map at the Pages root, which is the surface a first-time visitor actually hits. The README stays the repo (github.com) front page and the exhaustive reference — different surface, different grain (short taglines here vs full one-line descriptions there), so there is **no catalog to drift**: the index is a thin router, not a second catalog. The persona map and sourcing discipline remain canonical in the README, one click away.

**Posture.** Router-not-catalog — cards carry a ≤8-word tagline + a TOOL/READ pill, never the README's sentence, so the two surfaces don't hold the same text at the same grain. Reuses the house clay/cream palette + status-pill system verbatim from `claude-enterprise-architecture.html` (the anti-generic-AI signature, already `prefers-color-scheme`-aware); no webfont, no CDN, no framework — filtering is ~50 lines of vanilla JS over `data-*` attributes with real `<button aria-pressed>` chips, an empty-subgroup/empty-stage collapse, and a no-results state. No hardcoded artifact count (staleness liability). Stage vocabulary (5 stages) is a coarser decomposition of the spine's 7 decisions, ordered so they don't visibly fight. All 51 artifacts mapped to a stage (or the cross-cutting CTA/orient strip); every link verified to resolve (`.html` output ← `.md` source, the established house convention). Root `index.html` with no YAML front matter → Jekyll serves it verbatim (no theme layout) and it takes precedence over the README at the Pages root; **post-deploy, view-source the live root to confirm no theme chrome wrapped it and the README is no longer served there.** Independent expert review before build; ship-time render + filter verified on a local server.

### `usage-compliance-monitoring.md` (added 2026-07)

**Why it earns its place.** The "measurement / monitoring" space *looked* covered — four artifacts touch it — but each owns a different lens and all four leave the same hole. [`token-budget-governance.md`](../artifacts/token-budget-governance.md) is the **cost** lens on the Usage & Cost API; [`agent-observability-guide.md`](../artifacts/agent-observability-guide.md) is **app telemetry for the agents you build on Claude** (OTel, golden signals), not org-wide product usage; [`value-realization-guide.md`](../artifacts/value-realization-guide.md) is **ROI**; [`governance-overlay.md`](../artifacts/governance-overlay.md) §9 states the compliance **posture** ("audit trail — what to log") but is a design-time policy reference, not an operational monitoring layer. The hole is the **CISO / compliance-officer / IT-admin operational question**: now that employees use Claude, how do I *watch* it, *prove* policy to an auditor, feed it to the SOC, and support eDiscovery? That surface is real and was almost entirely undocumented here — **audit logs and the Compliance API weren't even feature-inventory rows** before this. User-requested ("the playbook needs a richer artifact on usage monitoring / compliance / analytics"); the CISO/admin center-of-gravity was the user's explicit choice among three framings.

**Why it's not in the original 8 (or prior additions).** Original scope treated monitoring as either cost (calculator) or the governance-overlay's policy reference. Empirically the *operational* admin surface is a distinct decision layer that only exists at Enterprise scale and only matured recently — the three-plane split (**Admin API** `sk-ant-admin` for org management, **Analytics API** `read:analytics` for usage, **Compliance API** scoped keys for content/eDiscovery/SIEM), the audit-log export mechanics, the 60+ DLP/SIEM partner ecosystem, and the on-by-default-2026-07-11 member-analytics flip are all current-surface facts a v1 couldn't have scoped. It sits beside `agent-observability-guide.md` as its **org-usage/compliance counterpart**: that one instruments *your workloads*, this one monitors *employees' product usage*.

**Posture.** Decision-frame first (three-plane distinction → data catalog → monitoring matrix → SOC wiring → eDiscovery → the line → failure modes → maturity → 5-question gate), failure-mode per section (house tone). **Sourcing held hard:** every platform fact graded **[H]** with a primary URL, live-verified 2026-07-12/13 by an independent research pass; the two secondary-only partner names (Varonis/Splunk) explicitly demoted to **[M]**; "no native content-DLP product" flagged as a reasoned-absence negative, not asserted as spec; relative-timestamp caveats carried; **BAA/ZDR routed to `governance-overlay.md`, never asserted here** (the [[verify-baa-from-source-table]] discipline). **Its spine is the purpose-limitation reconciliation** — the platform now exposes per-user analytics (on by default since 2026-07-11), per-user cost ranking, and individual conversation content, all legitimate for security but forbidden for productivity surveillance; the artifact holds the repo's existing bright line ([`workforce-change-guide.md`](../artifacts/workforce-change-guide.md) §4, [`value-realization-guide.md`](../artifacts/value-realization-guide.md)) with a two-consumers/two-rule-sets split and the works-council/employment-law gate, rather than quietly opening a surveillance vector. Adds four feature-inventory rows (Admin-API org-mgmt, Analytics API, Audit logs, Compliance API) with verify caveats. Independent review before ship. Cross-linked to governance-overlay (posture), token-budget (cost plane), agent-observability (sibling), workforce-change + value-realization (the boundary), incident-response (alert routing), enterprise-deployment (AWS-route plane caveat). Pinned to current surface; refreshed monthly.

### `enterprise-workspaces-guide.html` (added 2026-07)

**Why it earns its place.** [`enterprise-deployment-guide.md`](../artifacts/enterprise-deployment-guide.md) owns workspace *topology* (§3, one-workspace-per-environment) and [`token-budget-governance.md`](../artifacts/token-budget-governance.md) owns the *budget* boundary — but neither carried the workspace **lifecycle** (create → manage → archive, and the load-bearing fact that **there is no hard-delete**), the **five roles** and their least-privilege lever, or a **worked end-to-end enterprise** walking the whole arc. The gap surfaced while adding a §4 lifecycle summary to the deployment guide: the mechanics deserved more than a summary table, and no artifact answered the CISO/platform-lead's literal "how do we create, manage, archive, and decommission these — and what breaks?" in one visual place. This guide is that answer — the deep-dive the deployment-guide §4 routes into. User-requested ("a detailed guide answering what/why/how about workspaces, with visuals, from an end-to-end real-enterprise POV").

**Why it's not in the original 8 (or prior additions).** Original scope treated Console workspaces as a single line in the cost/governance tooling (segment teams, set caps). Empirically the *lifecycle* is a distinct decision layer with a non-obvious terminal edge — archive is irreversible, cascades to every key, and there is no delete — plus a five-role RBAC model and an org-vs-workspace limit-stacking rule a v1 never documented. It rides the same bar `enterprise-deployment-guide.md` cleared (a real architect decision routed to nothing) and completes it: that guide is the topology summary, this is the full what/why/how.

**Posture.** Decision-frame first, failure-mode per section (house tone). **Mixed visuals** — a hand-drawn hero in the `reference-architectures.html` color taxonomy + structured lifecycle / topology / RACI / decision-tree diagrams with hover tooltips, deviating to structured where process and matrix content read clearer (the same deliberate deviation `claude-enterprise-architecture.html` made). **Hybrid structure** — a what/why/how backbone with one fictional regulated enterprise (**Karekal Health**, labeled illustrative throughout) threaded as callouts through each stage, so the end-to-end-enterprise POV stays navigable and monthly-refreshable. **Sourcing held hard:** every workspace mechanic graded **[H]**, live-verified 2026-07-14 against `platform.claude.com/docs/en/manage-claude/workspaces` + the Admin-API doc + the Help-Center archive article; patterns explicitly split into Anthropic's *documented* use cases and this repo's *recommendations* (no observed-field-data claim); **BAA/ZDR/residency routed to [`governance-overlay.md`](../artifacts/governance-overlay.md), never asserted** (the [[verify-baa-from-source-table]] discipline). Adds the `manage-claude/workspaces` doc as a feature-inventory row; the three workspace rows + the Workspace-geo residency row now map here. Deep-dive companion to `enterprise-deployment-guide.md` §4 (topology summary → this), cross-linked to `token-budget-governance.md` (budget ladder), `governance-overlay.md` (the delete-is-retention reroute), `surface-rollout-matrix.md` (a workspace ≠ Projects / Cowork / Claude Code seats). Independent review before ship. Pinned to current surface; refreshed monthly.

### `legal-hold-ediscovery-runbook.md` (added 2026-07)

**Why it earns its place.** [`usage-compliance-monitoring.md`](../artifacts/usage-compliance-monitoring.md) §5 named eDiscovery + legal hold but carried it as a sub-section of the CISO monitoring artifact — the tradeoff statement, not the operational protocol. Its own §8 L2 maturity rung literally listed "write the eDiscovery + legal-hold runbook" as the next move. When a hold actually lands, Legal / Compliance + the executing admin need a step-by-step: what your retention / ZDR choices already decided is producible, what's retrievable per surface, which lever to pull (preserve vs delete), and the two ways to get it wrong (ZDR = can't produce; delete-under-hold = spoliation). That protocol had no home. User-requested ("break out a dedicated legal-hold / eDiscovery runbook").

**Why it's not in the original 8 (or prior additions).** The whole compliance-ops surface (audit logs, Compliance API) wasn't even a set of feature-inventory rows until `usage-compliance-monitoring.md` (2026-07); the legal-hold *operational* layer is one level deeper again — a reactive runbook, sibling to [`incident-response-runbook.md`](../artifacts/incident-response-runbook.md) (technical incidents) and [`model-deprecation-runbook.md`](../artifacts/model-deprecation-runbook.md) (migration), for the legal-request class. It clears the bar the same way those did: a real operational decision routed to nothing.

**Posture.** Runbook format (standing prep → scope-reality table → request-type matrix → 7-step response protocol → the two traps → failure modes → readiness gate), decision-frame first ("what your retention + ZDR choices already decided"), failure-mode per section (house tone). **Carve, not clone** (the advisor's blocking note): `usage-compliance-monitoring.md` §5 was *condensed* to the tradeoff + a forward-pointer so the two files don't hold the same operational prose to drift — the retention clocks appear compactly in §5 and as a full table here, the protocol lives only here. **Sourcing held hard, honesty foregrounded:** the load-bearing fact — **Claude has no named legal-hold primitive; preservation = *extend* retention (documented, [H]) + Compliance-API retrieve/delete** — is stated up front, and the undocumented pieces (retention *suspension* / per-matter hold, Compliance-API content query granularity, Public-Sector eDiscovery path) are graded **[inf]/[?]** and collected in an explicit "confirm with Anthropic" section, never asserted to a court. Makes **no new feature claims** — reuses four existing feature-inventory rows (Audit logs · Compliance API · Standard retention · ZDR), which now map here. A platform-mechanics-not-legal-advice disclaimer sits **up top and** in the footer (it tells people how to respond to litigation holds — the disclaimer is load-bearing, per advisor). **BAA/ZDR/residency routed to [`governance-overlay.md`](../artifacts/governance-overlay.md), never asserted.** Cross-linked to `usage-compliance-monitoring.md` (the planes it relies on), `governance-overlay.md` (retention / ZDR policy), `incident-response-runbook.md` (reactive sibling), `enterprise-data-boundaries.html`, `procurement-pack.md`. Independent review before ship; render-checked for the kramdown pipe-table gotcha. Pinned to current surface; refreshed monthly.

### `legal-hold-ediscovery-flow.html` (added 2026-07)

**Why it earns its place.** The user asked for visuals to hold reader attention on the legal-hold runbook. Markdown can't carry colour on GitHub (the repo's own render lesson), so — following the [`user-mindset-mindmap.html`](../artifacts/user-mindset-mindmap.html) / [`enterprise-data-boundaries.html`](../artifacts/enterprise-data-boundaries.html) visual-companion pattern — the flow ships as a standalone inline-SVG HTML page: the preserve-vs-delete fork (green = preserve/produce, red = delete) and the 7-step response spine (preserve **before** you scope), with the ZDR "can't produce" and spoliation traps called out. Clean warm palette, no JS, print-clean.

**Why it's not in the original 8.** Same as the mindmap/colour companions — a visual re-rendering rides in on an explicit user request for visuals. **Not new content** — it re-renders the runbook's fork + 7-step protocol into a diagram and introduces no new factual claims; named `-flow` (not `-runbook.html`) to dodge the Jekyll `.md`→`.html` collision the repo has hit before. The runbook itself also gained two in-file ASCII decision diagrams (fenced, GitHub-Pages-safe, like `incident-response-runbook.md`'s triage tree). Render-verified via the a11y tree (screenshot capture was unavailable in-session); pipe-gate clean.

### `service-accounts-guide.md` (added 2026-07)

**Why it earns its place.** "Service account" is referenced across four artifacts — [`claude-code-enterprise-config.md`](../artifacts/claude-code-enterprise-config.md) §5.1 (WIF wiring), [`enterprise-workspaces-guide.html`](../artifacts/enterprise-workspaces-guide.html) (the credential-axis matrix), [`agentic-threat-model.md`](../artifacts/agentic-threat-model.md) (excessive agency), [`token-budget-governance.md`](../artifacts/token-budget-governance.md) (per-workload attribution) — but *defined* in none. The credential-axis matrix even captions itself "this matrix is the map; that's the wiring," pointing at config §5.1 — map and wiring both exist; the plain concept anchor between them did not. User-requested ("do we address the service-accounts concept anywhere? — write the explainer"). This is that anchor: what a non-human identity is, which Claude credential is (and isn't) one, and the governance decision it forces — cross-cutting across the engineer (Claude Code tokens), the architect (WIF / workspace keys), and the CISO (attribution / offboarding), so no single existing host serves all three.

**Why it's not in the original 8 (or prior additions).** Original scope treated identity/credentials as implicit in the config + workspaces artifacts. Empirically the concept sits *across* them and is defined by none — the WIF row, the credential-axis matrix, and the `setup-token` discussion each assume the reader already knows what a service account is and why non-human identity matters. It is **not scope-completion** (no artifact *promised* a named-but-empty tier the way `workforce-change-guide.md` §3 promised the practitioner cheatsheet); it clears the bar the way [`enterprise-deployment-guide.md`](../artifacts/enterprise-deployment-guide.md) did — a concept a real reader asked for that routed to nothing but scattered wiring.

**Posture.** Decision-frame first — leads with *which identity authenticates which surface, person vs. machine*, not an IAM-101 "what is a service account" (which would fail the not-a-tutorial bar). The value is the **Claude-specific credential taxonomy** (workspace key · WIF `svac_` · Admin key · Claude Code `setup-token` · hyperscaler IAM); the load-bearing distinction is that a Claude Code `setup-token` is **subscriber-bound, not a service account** (§4) — the trap that puts automation on a person's credential. Failure-mode per section (house tone). **Makes no new feature claims** — re-anchors existing feature-inventory rows (Workload Identity Federation · workspace lifecycle/keys · Admin API) via their `Used in artifacts` column, like [`legal-hold-ediscovery-runbook.md`](../artifacts/legal-hold-ediscovery-runbook.md). **Asserts no BAA/ZDR/residency** — routes to [`governance-overlay.md`](../artifacts/governance-overlay.md). Routes the "turn `setup-token` off centrally" nuance to config §5.1 rather than restating it (cross-file-consistency). Cross-linked to the four artifacts it anchors + governance-overlay + usage-compliance-monitoring. Pinned to current surface; refreshed monthly.

### `service-accounts-architecture.html` (added 2026-07)

**Why it earns its place.** The visual companion to [`service-accounts-guide.md`](../artifacts/service-accounts-guide.md) — the credential taxonomy (§3) and its three focal decisions (§4 the `setup-token` trap, §6 the hyperscaler attribution gap, §5 least-privilege) drawn as one **identity architecture** over a realistically complex enterprise deployment. Markdown can't carry colour on GitHub, so — following the [`user-mindset-mindmap.html`](../artifacts/user-mindset-mindmap.html) / [`legal-hold-ediscovery-flow.html`](../artifacts/legal-hold-ediscovery-flow.html) / [`enterprise-data-boundaries.html`](../artifacts/enterprise-data-boundaries.html) visual-companion pattern — the diagram ships as a standalone inline-SVG HTML page. User-requested ("add visuals depicting a complex enterprise deployment architecture").

**Why it's not in the original 8.** Same as the other visual companions — a visual re-rendering rides in on an explicit request for visuals. **Not new content** — it re-renders the guide's §3–§6 and introduces no new factual claims; named `-architecture` (not `service-accounts-guide.html`) to dodge the Jekyll `.md`→`.html` collision the repo has hit before.

**Posture.** **Identity lens, not topology** (the scope guard): it *does* depict a genuinely complex deployment — prod/non-prod workspaces, teams, the hyperscaler path — but every node is annotated with its credential + person-vs-machine, and a banner + cross-link routes *layout* decisions to [`enterprise-deployment-guide.md`](../artifacts/enterprise-deployment-guide.md) and [`claude-enterprise-architecture.html`](../artifacts/claude-enterprise-architecture.html) (same topology can appear; different question). **Reuses Karekal Health** (the illustrative enterprise from [`enterprise-workspaces-guide.html`](../artifacts/enterprise-workspaces-guide.html)) for cross-artifact consistency, labelled illustrative. **Structured inline SVG** (not the hand-drawn house style, which stays scoped to `reference-architectures.html` + `enterprise-data-boundaries.html`) — the credential-flow density reads clearer structured, the same deliberate deviation `claude-enterprise-architecture.html` made; person = warm, machine = teal, trap = red, hyperscaler = slate; no JS, prints clean. Decision-framed: the `setup-token` trap, attribution gap, and least-privilege are focal callout cards, not just boxes. **BAA/ZDR/residency routed to [`governance-overlay.md`](../artifacts/governance-overlay.md), never asserted.** Render-verified before wiring (literal-hex fills for cross-browser safety; no viewBox clipping — checked in-browser). Bidirectionally linked with the guide.

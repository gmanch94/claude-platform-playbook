...
| [`claude-code-enterprise-config.md`](artifacts/claude-code-enterprise-config.md) | Reference (admin templates) | Three deployable templates — reference org instructions, `managed-settings.json`, enterprise `CLAUDE.md` — on the enforce-vs-guide spine (managed settings enforce client-side; managed CLAUDE.md only guides). Verbatim managed-settings paths per OS, the five-source delivery precedence (one source wins), deny-beats-allow, `forceLoginOrgUUID` org lock, the sandbox/egress OS-level perimeter, and MCP/hook/plugin supply-chain governance; every field carries its failure-mode-if-omitted (managed security fields fail closed). Does not assert BAA/ZDR — routes to `governance-overlay.md`. |

...
| [`claude-code-101.md`](artifacts/claude-code-101.md) | Operational (practitioner) | Field guide for the engineer who just installed Claude Code. Opens with a cautiously-optimistic-expert operating posture (plan→solve→verify, mandate adversarial review, stay token-frugal), then the daily edit-verify loop, the 6 permission modes, model aliases (incl. `opusplan`), CLAUDE.md craft (imports don't save context; it guides, doesn't enforce), context management, subagents, headless/CI, worktrees — each with its gotcha and when-to-use. Curated judgment over the docs; models referenced to `feature-inventory.md`, not restated. |
| [`claude-code-enterprise-config.md`](artifacts/claude-code-enterprise-config.md) | Reference (admin templates) | Three deployable templates — reference org instructions, `managed-settings.json`, enterprise `CLAUDE.md` (carrying a plan→solve→verify, cautiously-optimistic-expert operating posture) — on the enforce-vs-guide spine (managed settings enforce client-side; managed CLAUDE.md only guides). Verbatim managed-settings paths per OS, the five-source delivery precedence (one source wins), deny-beats-allow, `forceLoginOrgUUID` org lock, the sandbox/egress OS-level perimeter, and MCP/hook/plugin supply-chain governance; every field carries its failure-mode-if-omitted (managed security fields fail closed). Does not assert BAA/ZDR — routes to `governance-overlay.md`. |

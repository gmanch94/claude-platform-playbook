...
| [`claude-code-101.md`](artifacts/claude-code-101.md) | Operational (practitioner) | Field guide for the engineer who just installed Claude Code. The daily edit-verify loop, the 6 permission modes, model aliases (incl. `opusplan`), CLAUDE.md craft (imports don't save context; it guides, doesn't enforce), context management, subagents, headless/CI, worktrees — each with its gotcha and when-to-use. Curated judgment over the docs; models referenced to `feature-inventory.md`, not restated. |

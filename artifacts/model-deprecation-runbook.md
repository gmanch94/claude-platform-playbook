...
| 2 | **Successor selection + param-parity check** | Pick the candidate tier per [`model-selection-guide.md`](model-selection-guide.md). Then check parameter parity — behavior contracts differ across generations. Real example: Opus 4.8 accepts adaptive thinking only and **rejects non-default `temperature` / `top_p` / `top_k` with HTTP 400** ([`docs/feature-inventory.md`](../docs/feature-inventory.md)). A lift-and-shift of request code can fail before quality is even in question. **Live worked example — Opus 4.8 → Opus 5 (2026-07-24):** a *silent* parity break, worse than a 400. Requests that ran thinking-less on 4.8 now run **with thinking on** by default on Opus 5, and `max_tokens` caps thinking **plus** response text — so an unchanged request doesn't error, it **truncates**. Parity checks must cover defaults that changed, not just parameters that are rejected. | Architect | Candidate named; request-shape diffs listed |
...
| 4 | **Cost re-forecast** | Three deltas, not one: (a) list-price difference; (b) **tokenizer drift** — Sonnet 5's updated tokenizer maps the same input to ~1.0–1.35× more tokens than Sonnet 4.6, so identical list price can still mean a higher bill; (c) cache-floor changes (Opus 4.8 lowered the minimum cacheable prompt to 1,024 tokens from 4,096 on 4.7 — cache eligibility shifts; **Opus 5's floor is unstated — verify, don't assume it carries over**); (d) **thinking-token spend** — if the successor turns thinking on by default (as Opus 5 does), output tokens per request rise even at an identical list price, and `effort` becomes a live cost dial. Re-run [`cost-calculator.html`](cost-calculator.html) per workload; update the budget per [`token-budget-governance.md`](token-budget-governance.md). | FinOps + workload owner | New $/task accepted by budget owner |

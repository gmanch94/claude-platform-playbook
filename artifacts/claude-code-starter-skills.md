...
**As of 2026-07.** Pin to current model surface (Opus 5 / Sonnet 5 / Haiku 4.5). Companion to [`claude-code-adoption-guide.md`](claude-code-adoption-guide.md). See [`../docs/feature-inventory.md`](../docs/feature-inventory.md) for the canonical Skills + plugin surface inventory.
...
5. **Trim each template to what your team actually does differently.** These are written full so you can see the shape; ship them shorter. A Skill is a guide that lets Claude find *your* knowledge when it's relevant — your merge bar, your migration rules, your on-call format — not a rulebook for how to write code. Over-constraint costs twice: it narrows how the model approaches the problem, and it competes with your `CLAUDE.md` and the developer's own prompt for authority. Keep the hard rules where a wrong answer is expensive (the migration guard's block list, the review's merge bar) and let judgment cover the rest. `[M — claude.com/blog/the-new-rules-of-context-engineering-for-claude-5-generation-models, 2026-07-24]`
6. **Watch the listing, not just the file.** Every installed Skill contributes to a listing the model reads each session, so the pack has a standing cost before any Skill fires. `/doctor` estimates that cost and names the biggest contributors; `skillOverrides` (**v2.1.129+**) hides or collapses one without editing its `SKILL.md`. For a long Skill, split the detail into helper files so it loads only when needed rather than every time the Skill triggers.
...
`© gmanch94 · CC-BY-4.0 · As of 2026-07. Verify Skills surface at docs.claude.com/en/docs/agents-and-tools/skills.`

...
**As of 2026-07.** Pin to current model surface (Opus 5 / Sonnet 5 / Haiku 4.5) — refresh monthly with [`../docs/feature-inventory.md`](../docs/feature-inventory.md).
...
| **LLM as sole judge in LLM-as-judge eval pipelines** | Same-family models (e.g., Claude judging Claude outputs) share systematic biases — the judge preferentially passes outputs that match its own style and reasoning patterns. High false-pass rate on adversarial inputs; low false-pass on outputs from different model families. Stochastic judge on a stochastic system produces compounding noise, not a reliable quality gate. | Multi-judge ensemble (Claude + GPT-class + human spot-check on 5–10% of eval runs). Use deterministic checks (regex, schema validation, citation match) wherever possible. Reserve Claude-as-judge for rubric categories that resist deterministic evaluation (tone, coherence, helpfulness). See [`eval-starter-pack.md`](eval-starter-pack.md) adversarial + grounding templates. | [`eval-starter-pack.md`](eval-starter-pack.md); NIST AI RMF Measure 2.5 (bias evaluation) |

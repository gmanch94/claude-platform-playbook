...
| **Cowork** | Non-technical knowledge workers (analysts, ops, legal, finance) | **Paid only** (Pro/Max/Team/Ent) | **Desktop app** (local files) · **web/mobile beta** | **Takes real actions on connected files; session runs remotely on Anthropic's servers.** Desktop app required for local-folder access. Folder-scoped + egress-controlled + review-before-act. **Not BAA-covered as of 2026-07 — no PHI** (coverage expands; confirm current scope with your rep at signing). [H] | [`cowork-adoption-guide.md`](cowork-adoption-guide.md) |
| **Claude Design** | Product, UX, design | **Pro / Max / Team / Enterprise** — **default OFF on Enterprise**; Team/Ent gate via the Labs toggle | Inside chat | **Beta.** Assets stored persistently; **no data residency**; not BAA-covered as of 2026-07 (beta — may change; confirm at signing). Org-wide enable without a design system → generic output. **It reaches Pro/Max seats with no org gate** — the seat-plan question, not just an admin toggle. [H] | This matrix |
...
3. **Claude Design — gate before you broaden, and note where you *can't*.** On Team/Enterprise it's enabled via a toggle under *Anthropic Labs* in Org settings, controllable with custom roles, and ships **default off on Enterprise**. But it is **available in beta on Pro and Max too**, where there is no org admin to gate it — so if your people hold individual paid seats, the surface is reachable regardless of your org posture. That makes it a seat-plan and policy question ([`subscription-selection-guide.md`](subscription-selection-guide.md)), not only an admin-toggle one. Managing the design *system* is a separate `Claude Design Admin` permission from using the surface. Two flags: (a) **org-wide enable without a design system produces functional-but-generic output** — Anthropic says so in its own rollout phases; (b) uploaded assets (brand guidelines, screenshots) are **stored persistently** and there's **no data-residency support yet**, and it's beta so **not BAA-covered**. Follow Anthropic's published phases; add the asset-governance check. → *Failure mode:* a residency-bound or brand-sensitive org enabling Design org-wide in beta. [H — [support.claude.com Design admin](https://support.claude.com/en/articles/14604406-claude-design-admin-guide-for-team-and-enterprise-plans)]
...
| Has product/design as the pilot function | Claude Design (any paid plan; default off on Ent) | Cowork until file-governance is owned |

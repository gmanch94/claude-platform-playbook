...
| Where is our data processed / stored? | First-party API + Claude Platform on AWS: per-request inference geo via `inference_geo` (`global` default or `us`; Opus 4.6+ and Sonnet 4.6+); at-rest + endpoint processing controlled by **Workspace geo**. Bedrock/Vertex/Azure AI Foundry use the hyperscaler's own region selection, not `inference_geo`. | `[gov §5]` `[inv]` |

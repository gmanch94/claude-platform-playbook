...
| Where is our data processed / stored? | First-party API: per-request inference geo via `inference_geo` (`global` default or `us`; Opus 4.6+); at-rest + endpoint processing controlled by **Workspace geo**. Bedrock/Vertex use the hyperscaler's own region selection, not `inference_geo`. | `[gov §5]` `[inv]` |
...
| Which features are BAA-covered? (Enterprise product) | Chat, Projects, Artifacts, file creation & code execution **excluding network access / external websites**, Voice, Web search, Research, Skills. (Skills is covered on the Enterprise *product* surface but not as the first-party-API "Skills API" below — surface split, not a contradiction.) Eligible list grows — **verify the current list in the Implementation Guide at signing.** | `[gov §4]` · verify: Implementation Guide |

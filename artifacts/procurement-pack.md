...
| Which features are BAA-covered? (Enterprise product) | Chat, Projects, Artifacts, file creation & code execution **excluding network access / external websites**, Voice, Web search, Research, **Skills** (admin-enabled — confirmed covered per the HIPAA-ready "Feature availability" list, support.claude.com, current 2026-07). Eligible list grows — **verify the current list in the Implementation Guide at signing.** | `[gov §4]` · verify: Implementation Guide |
...
| Is Cowork / Claude Design BAA-covered? | **Not as of 2026-07.** Both are BAA-excluded (Cowork GA but excluded; Design is beta) — but the HIPAA-ready covered set expands over time, so confirm current scope with your Anthropic rep at signing. No PHI on either surface today. | `[gov §4]` `[inv: product surfaces]` |

...
**Sourcing:** every behavior below is verified against [docs.claude.com/en/docs/claude-code](https://docs.claude.com/en/docs/claude-code) as of 2026-07-11 `[H]`, except where a line is marked `[M]` — those come from an Anthropic engineering post rather than the docs and are graded inline. Version-pinned specifics (feature X "requires v2.1.N") change fast — the linked docs are canonical; treat this guide as the *judgment layer* over them. Model pricing/tiers are **not** restated here — see [`../docs/feature-inventory.md`](../docs/feature-inventory.md), the repo's single source of truth.
...
- **Prefer progressive disclosure to one long file.** The instinct to make `CLAUDE.md` the central repository of everything the team knows — on the theory that Claude won't find it otherwise — is the common failure `[M]`. It will usually find it: point at a skill or a path-scoped rule and it loads when relevant. If you have a verification ritual, make it a verification skill and reference it in one line. The caveat is the skill listing, not the mechanism — when the listing overflows its budget, descriptions get shortened and the keywords a skill matches on can be stripped (§9).
...
- **Set up a repo:** `/init` (scaffold CLAUDE.md) · `/doctor` (setup checkup; estimates what your skill listing is costing in context, and on **v2.1.206+** proposes trims for a checked-in `CLAUDE.md`) · `/memory` · `/mcp` · `/permissions`
...
**Write skills as guides, not rulebooks** `[M — claude.com/blog/the-new-rules-of-context-engineering-for-claude-5-generation-models, 2026-07-24]`**.** A skill's job is to let Claude find *your* knowledge when it's relevant — your review gates, your deploy ritual, the thing your team does differently. Over-constraining is the common mistake: a never/always rule narrows how the model approaches the problem, and it competes with your `CLAUDE.md` and your own prompt for authority. Reserve hard rules for the places where a wrong answer is expensive. Split a long skill into several files rather than one wall of text — the body loads when the skill fires, the helper files load only if needed. Every skill also sits in a listing the model reads each session, and an oversized listing gets its descriptions shortened, which can strip the keywords a skill needs to match on `[H]`; `/doctor` tells you what the listing costs and which skills dominate it.

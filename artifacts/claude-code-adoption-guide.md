...
**As of 2026-07.** Pin to current model surface (Opus 5 / Sonnet 5 / Haiku 4.5). See [`../docs/feature-inventory.md`](../docs/feature-inventory.md) for canonical Claude Code surface inventory.
...
A CLI tool (also runnable in VS Code / JetBrains / desktop / web) that runs an agentic loop in a developer's repo. It has built-in tools (file edit, bash, grep, web fetch, git, sub-agents) and extensible surfaces (slash commands, hooks, Skills, MCP servers, plugins). Default model is Sonnet 5; escalate to Opus 5 for hard problems; Haiku 4.5 for triage. Settings hierarchy is user → project → local (each shadows the previous). Headless mode runs the same loop in CI.

---

## The operating posture to instill

Rollout is a working style, not just tooling. Bake this into your onboarding and PR norms — and into the team plugin's `CLAUDE.md` for the subset that belongs in a machine-read file (see the note after the list) — so every engineer drives Claude like a **distinguished engineer**: optimistic the task is doable, rigorous about proving it's done.
...
Hand engineers [`claude-code-101.md`](claude-code-101.md) (the individual-practitioner version); the org-wide managed form is in [`claude-code-enterprise-config.md`](claude-code-enterprise-config.md).

**Instil it in people first, and write down only the part the model needs.** Most of the list above is how you want *engineers* to work, and that part earns more in onboarding, the PR template, and review norms — where it changes human behavior — than as text loaded into every session. The subset that belongs in the team plugin's `CLAUDE.md` is smaller than it looks, and phrasing decides it: Anthropic's own guidance for the Claude 5 generation was to drop worst-case never/always rules in favor of guidance the model can apply with judgment (its own prompt replaced "default to writing no comments … never write multi-line comment blocks" with "write code that reads like the surrounding code"). `[M — claude.com/blog/the-new-rules-of-context-engineering-for-claude-5-generation-models, 2026-07-24]` Write the rule if you have watched your repos get it wrong, write it as a convention rather than a prohibition, and check what generation your fleet actually runs before assuming judgment covers it. Also remember every scope's `CLAUDE.md` loads together: a team file and a managed file that disagree cost the model reasoning and pick a winner nobody chose ([`claude-code-enterprise-config.md`](claude-code-enterprise-config.md) §3).
...
`© gmanch94 · CC-BY-4.0 · As of 2026-07. Verify Claude Code surface at docs.claude.com/en/docs/claude-code.`

...
**Instil it in people first, and write down only what the model gets wrong.** Most of the list above is how you want *engineers* to work — that part belongs in onboarding, the PR template, and review norms, where it changes human behavior. Only a subset needs to be a line in a `CLAUDE.md`, and the subset is smaller than it looks: current-generation models already read the whole request, verify before claiming, and match the surrounding code's conventions without being told, and Anthropic deleted its own versions of those instructions when it cut Claude Code's system prompt for the Claude 5 generation. `[M — claude.com/blog/the-new-rules-of-context-engineering-for-claude-5-generation-models, 2026-07-24]` What earns a written line is what the model can't infer — your review gate, your escalation path, your cost policy — plus anything you have actually watched go wrong in your repos. Write the rest as team norms, and remember every scope's `CLAUDE.md` loads together: a team file and a managed file that disagree cost the model reasoning and pick a winner you didn't choose ([`claude-code-enterprise-config.md`](claude-code-enterprise-config.md) §3).

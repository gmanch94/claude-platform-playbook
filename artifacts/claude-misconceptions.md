...
> **How to read this.** Format: **Myth** → **Reality** → **What you'd mis-decide** → **Cite**. Pinned to the current model surface (Opus 5 / Sonnet 5 / Haiku 4.5). Refreshed monthly against [`docs/feature-inventory.md`](../docs/feature-inventory.md) — the single source of truth.
...
- **Reality.** The long-context surcharge has been eliminated on Opus 5, Opus 4.8, Opus 4.7, Opus 4.6, and Sonnet 4.6. A 900K-token request bills the same per-token rate as a 9K request on these models. Older / legacy models still apply the 200K break — verify per model. Sonnet 5's long-context pricing isn't separately confirmed — its published rate is flat per-MTok with no long-context break-point mentioned, but verify before relying on it for a 900K-token workload.

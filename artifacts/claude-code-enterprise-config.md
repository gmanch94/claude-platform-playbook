...
- **Cut the lines that restate generic good-model behavior; keep the ones that encode your organization.** This is the split worth being deliberate about. "Read the whole request before acting," "don't claim it passes without running it," "match the surrounding code's comment density" — current-generation models do these unprompted, and Anthropic deleted its own versions of exactly these when it removed "over 80%" of Claude Code's system prompt for Opus 5 and Fable 5 "with no measurable loss on our coding evaluations." `[M — same post]` **Carry the condition when you quote that number:** it is the vendor's own prompt measured on the vendor's own coding evals, not a promise about your file. What survives the cut is what a model *cannot* infer — your review gates, your escalation path, your data classifications, your model-cost policy. The same post's positive rule points the same way: guidance is best when it encodes "opinions, knowledge, or best practices that are particular to you, your team, or product."

Where a kept rule is long, prefer a reference over an inline block: a managed skill, or a link to the internal doc, so the detail loads when it's relevant rather than in every session on the machine. `/doctor` (**v2.1.206+**) will propose trims on a checked-in `CLAUDE.md` along the same line — it cuts what the model can derive from the codebase and keeps pitfalls, rationale, and conventions that differ from tool defaults. Run it against your draft; treat the output as a proposal.
...
**Which blocks above survive the cut.** *Data boundaries*, *How we work*, *Security posture*, and *Model + cost* are org-specific by construction — a model cannot infer your classifications, your review policy, or your spend expectations. Keep them. *Operating posture* is the mixed one: "mandate an adversarial review before merge" is your policy and stays, while the general craft lines (respond before reacting, verify by running it, don't answer from memory, name the failure mode) are the class current models already do and Anthropic deleted from its own prompt. Ship those only if you have watched your fleet get them wrong — an unearned rule is not free, it competes with the rules you care about. If you keep the whole posture block, know that you are spending your ~200 lines on it.

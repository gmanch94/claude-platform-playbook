...
| `network.allowAllUnixSockets` (the Linux / WSL2 equivalent — "the only way to permit Unix sockets, since it skips the seccomp filter") | **cannot be scoped to a path**, so it admits `docker.sock` along with every other socket. Strictly blunter than the macOS key above |

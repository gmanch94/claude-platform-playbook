...
### When review round 2 returns MORE criticals than round 1, the fix is to cut scope, not to specify harder

2026-07-27. A design doc went through two rounds of paired blind review (factual lens, operator lens). Round 1: "do not ship," 4 CRITICAL. Applied every finding in a substantial rewrite. Round 2: **5 CRITICAL and 17 HIGH — more than round 1** — and every CRITICAL was inside machinery the rewrite had just *added*. Three were self-contradictions the rewrite introduced: a primitive both mandated in one section and banned in another with no component owning it; a proposal lifetime rule that made its own mechanism livelock (execute at the next period boundary, expire at the next period boundary); and an append-only store that a governance section separately required to support erasure. A fourth was a guard that could not fire, because the field it watched was a category enum that reads identically on both sides of the event it was watching for.

**The mechanism.** Each round's findings are fixed by adding a mechanism, and each new mechanism arrives underspecified, contradicts something written before it, and needs its own detector. Reviews measure the *document*, so a document that grows 40% between rounds gets more findings, and it reads as diligence right up until you plot the count. **The round-over-round CRITICAL count is the signal; a single round's count is not.** Rising means the artifact is inflating faster than it is closing, and no amount of further specification reverses that — the next round will find the defects in whatever the next rewrite adds.

**The move that actually converged it.** v3 opened by *deleting* a whole subsystem (a secondary data path that cost a second credential, a second rate budget, an unfixable provenance gap and a documented vendor blind spot, to produce a signal the primary path already covered). That closed four findings outright with no new mechanism. Then it added one structural section — an explicit component inventory naming what the doc specifies and what it deliberately does **not**, with interface contracts for the rest — because "referenced in four places, owned by nothing" was the single most repeated finding across both rounds, appearing four separate times in the round-2 report against a document that had itself charged the previous version with exactly that defect.

**The operating rules.** (1) Count CRITICALs per round and act on the *derivative*. (2) When it rises, the next revision's first section is a **cut**, not an addition — YAGNI is a review-convergence tool, not just a design preference. (3) Give any artifact with more than a handful of moving parts an explicit **scope-boundary section**; an unowned mechanism is the most reliably-recurring defect class in multi-part designs, and only a named boundary retires it. (4) Reviewers who have not seen the conversation will faithfully review the mechanism you added, not the simpler design you could have written — **the decision to cut is yours alone, and no review round will hand it to you.**

### An uncertainty register is where the negative-claim rule breaks, because "searched in full" is the one claim nobody checks

2026-07-27. A design doc for a Claude Enterprise spend-limit automation shipped a `[?]` register — six open questions, each written in the house style that names what was searched so the next reader does not redo it. Item 2 read: *"What `period` values exist besides `monthly`? Only `monthly` appears in the guide's examples."* The register's own preamble said item 1 had "searched `manage-claude/spend-limits-api` **in full**." The guide carries a dedicated **Period** section answering the question outright — `monthly` is the only currently supported period, spend resets 00UTC on the first, treat `period` as an open set — and the reference the item defers to carries the enum. A blind reviewer found it in one fetch. Two of the other five items were also answerable from sources already named in the item text.

**Why this instance is worse than the earlier ones.** The prior misses (2026-07-25, twice) were bare negative claims in prose. This one was inside the very structure built to catch them. A register *looks* like the output of a search — it has an item, a scope, a named source, a grade — so it passes self-review harder than a naked sentence would. It converts an unasked question into a documented-looking artifact. Line 46 above already names the shape ("writing the search terms into the artifact is not a substitute for following the link — it documents the diligence without performing it"); the register is that failure with a schema around it.

**The operating rule, in two parts.** (1) **Attempt the resolution before writing the item.** `[?]` is the *result* of a failed search, never a hedge written in place of one. If the item names a source to check, fetch that source now — the cost of writing the item exceeds the cost of resolving it in most cases, which is the tell that the item should not exist. (2) **"Searched X in full" is a claim about your own conduct, and it is load-bearing** — a future reader skips the search on the strength of it. Only write it when a full fetch happened in the current session, and name the fetch, not the intent. A register that carries one false diligence claim is worth less than no register, because it launders the unsearched items sitting next to it.

**It then recurred twice more in the same document, and the third instance is the one to remember.** Revision 3's register claimed the docs "do not state the scope required to `GET /v1/organizations/spend_limit_increase_requests`." The scope is stated twice in a guide that revision listed as `[H]`-verified — and it was also stated **in that same revision's own credential table, seven lines above the claim**, which read "`read:spend_limits` … list increase requests." So the refuting evidence was not one link away or one page away; it was on the same screen, written by the same author, in the same pass. (The second instance, one revision earlier, generalized a "contact your account team to raise this limit" sentence from one API's page onto a different API's page that says no such thing.)

**What that rules out.** It is not a retrieval problem, a context problem, or a stale-snapshot problem — no search would have helped, because the answer was already in the document. It is that **the register is written in a different mode than the body.** The body asserts and therefore feels checkable; the register hedges, and a hedge reads as the safe option, so it is composed from recollection of what felt unsettled rather than from the text. The fix is mechanical: **before shipping a register, grep your own artifact for every term in each item.** An item refuted by the file it lives in is the cheapest possible catch and the most embarrassing possible miss.

Corollary for review briefs: **brief a factual reviewer on the register explicitly.** A reviewer told to check the document's claims will check the claims and treat the open-questions section as the author's honest self-report rather than as claims in scope. Both round-1 reviewers here were given the register as a target, and that is the only reason it was caught.

### A source's worked example can be the inverse of the paraphrase you're about to write — quote the before AND the after

2026-07-27. Landing an Anthropic engineering post's context-engineering guidance, the mechanics layer was verified line-by-line against `docs.claude.com` and came back clean on every claim: version pins, `/doctor` behavior, `disableBundledSkills` semantics, the 200-line rule, all four `skillOverrides` states. The defect was one layer up, in the *interpretive* sentence built on top: "current-generation models do these unprompted, and Anthropic deleted its own versions of exactly these," listing three examples. The post shows the opposite for the example it actually works through — the old prompt's "default to writing no comments … never write multi-line comment blocks" was **replaced** by "write code that reads like the surrounding code: match its comment density, naming, and idiom." The vendor kept the subject and dropped the prohibition. The other two examples appear nowhere in the post. An admin following the shipped wording would have deleted from their managed `CLAUDE.md` the exact line the vendor kept in its own — and the sentence carried `[M — same post]`, so a reader who checked the cite would find the contrary.

**Why it survives care.** Verification instinct runs on *facts* — a version number, a key name, a quoted string — and every fact here passed. A paraphrase of an argument has no fact-shaped surface to check, so it never enters the queue. The tell is grammatical again: **"X deleted / dropped / stopped doing Y" is a claim about a source's behavior, and it needs the before AND the after in front of you.** Reading only the "Now:" half of a then-and-now section is how you conclude the thing was removed when it was reworded. Same for a number: "removed over 80%" hid that part of the 80% was *relocated into skills*, which the same post says plainly one section away — the source published the matching cell and the summary dropped it.

**The operating rule:** when a document's recommendation rests on what a source did, paste the source's before-state and after-state next to each other before writing the sentence. If you can only find one of the two, you have not read enough to make the claim. And brief the reviewer on the *interpretive* layer explicitly — a factual reviewer told to check claims will check the facts and can walk past the argument they support.

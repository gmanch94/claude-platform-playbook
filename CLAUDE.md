...
└── artifacts/
    ...
    ├── claude-security-plugin.md        Which of the FOUR differently-named "Claude security" things you're being sold · leads on the **Claude Security plugin** for Claude Code (beta, `/claude-security`) = on-demand multi-agent deep scan of a repo/diff → independently-reviewed findings report (`CLAUDE-SECURITY-RESULTS.md`/`.jsonl` + revision stamp tying report to commit) → patches drafted in a scratch repo copy · exact requirements gate (CC v2.1.154+ · paid plan for dynamic workflows · `python3` 3.9.6+ stdlib-only · git for change-scan/patch) · six-layer defense-in-depth ladder (guidance plugin → `/security-review` → this → Code Review → managed product → your CI) · disambiguates it from the security-**guidance** plugin (2026-05, free, regex), the managed **Claude Security** product (2026-04, Enterprise), and the **Compliance API** partner network (2026-05, usage governance not code scanning) · load-bearing correction: the 30–40%-fewer-PR-security-comments figure belongs to the guidance plugin, NOT this one · date graded [M] (X announcement, no dated blog post); mechanics [H] vs code.claude.com verified 2026-07-23 · BAA/ZDR/residency routed to governance-overlay
    ├── claude-security-plugin-stack.html  Visual companion to claude-security-plugin.md · the six-layer code-security stack with the newly-announced layer highlighted + three-jobs / requirements-gate / failure-mode cards + the four-way disambiguation table · structured inline SVG (literal-hex fills), no JS, prints clean · NOT new content (re-renders §1–§6) · named -stack to dodge the .md→.html Jekyll collision · structurally render-verified (40 texts, 0 viewBox overflow, 0 clipped nodes, 0 unresolved var() in SVG attrs)

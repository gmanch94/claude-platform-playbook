...
└── artifacts/
    ...
    ├── cowork-101.md                    Cowork practitioner field guide · operator twin of claude-product-101/claude-code-101 · the task lifecycle (brief→plan→review-gate→work→deliverable→verify) · daily loop done well · folder-as-privilege-dial · review-before-act in practice (ask-first vs hard-stop · "can it be undone quietly?" test) · sub-agents · long-running/scheduled tasks · 4 worked examples (analyst reconcile · ops deck · finance de-dup · research extract-w/-citations) each w/ brief+plan-shape+where-to-check+failure-mode · one governance boundary line (no PHI in a connected folder; BAA-excluded) routed to governance-overlay · rollout depth routed to cowork-adoption-guide · pointer-not-repeat vs product-101 §4 + user-mindset-cheatsheet · every mechanic traced to the feature-inventory Cowork row
    ├── cowork-101-workflow.html         Visual companion to cowork-101.md · 3 inline-SVG workflow diagrams (task lifecycle w/ the 2 checkpoints · review-gate "undone quietly?" → ask-first/hard-stop + invoice example · folder privilege dial: tight-scope vs drive-root blast radius + remote-isolation-protects-your-computer-not-connected-folders note) · literal-hex fills (no var() in SVG attrs) · no JS · prints clean · NOT new content · named -workflow to dodge the .md→.html Jekyll collision · structurally render-verified (3 SVGs · 60 texts · 0 overflow · 0 unresolved var())
    ...
    ├── system-card-readout.md           Reading a Claude system card as an executive · the source class the launch post + docs + pricing page don't carry (RSP determination + protection tier · safeguard POLICY changes · measured adversarial robustness · honesty profile) · 6 questions leadership asks × where each is answered × what a good answer looks like · **5 rules for quoting the numbers** (safeguards-disabled measurement · helpful-only variants · attacker-generous setups · bounded 0%/none-found claims · vendor-benchmarked = [M]) · Opus 5 as the WORKED INSTANCE (§4 = the only section that expires — swap per release) · 5 actions each w/ owner + failure-mode-if-skipped · no new feature claims (re-anchors the feature-inventory Compliance rows) · BAA/ZDR/residency routed to governance-overlay · not legal advice
    ├── claude-security-layers.md        The CHOOSER across Anthropic's FIVE code-security layers (+ your own CI beneath them = 6 rows) + 3 controls outside the stack (2 adjacent, 1 underneath) · §1 four-way disambiguation (deep-scan **plugin** vs security-**guidance** plugin vs managed **Claude Security** product vs **Compliance API** partner network — four price points, four buyers) · then layer-by-layer with mechanics + gate + failure mode: [1] guidance plugin (free, all users; 3 hook checkpoints SessionStart/UserPromptSubmit/PostToolUse-edit/Stop/Bash-commit; additive `claude-security-guidance.md` + `security-patterns.yaml`; **guidance ≠ enforcement — never blocks a write**; own-shell commits unreviewed; 20/hr cap; model-backed reviews SKIP on Bedrock/GCP) · [2] `/security-review` (built in, no gate, nothing durable) · [3] `/claude-security` deep scan (beta, paid plan, CC v2.1.154+, python3 3.9.6+, git; 3 jobs; `CLAUDE-SECURITY-RESULTS.md`/`.jsonl` + revision stamp; patches in a scratch repo copy) · [4] Code Review (**research preview · Team/Ent · ~$15–25 per review on usage credits, NOT plan usage · UNAVAILABLE under ZDR**) + `/code-review ultra` (free runs Pro 3 · Max 3 · Team/Ent none; **unavailable under ZDR and on Bedrock/GCP/Foundry** → silently downgrades to a local review) · [5] managed product (Enterprise, **premium seats**, consumption billing, 4 setup gates, scans minutes→hours, scope-to-a-directory advised) · [6] your own SAST/SCA (keep) · §9 adjacent-and-underneath (THREE, not two): runtime containment (`/sandbox`) → agentic-threat-model; usage governance (Compliance API) → usage-compliance-monitoring; **model-level cyber classifiers = the floor under all six layers** — 2026-07 source-code-vuln unblock at GA (binaries still blocked) + Cyber Verification Program (Enterprise can apply to unblock pentesting) · §10 turn-it-on decision table · §11 gates matrix · load-bearing correction: the 30–40%-fewer-PR-security-comments figure belongs to the GUIDANCE plugin, not the deep-scan one · deep-scan date [M] (X announcement, no dated blog post); all mechanics [H] vs code.claude.com verified 2026-07-23 · BAA/residency/retention routed to governance-overlay (only the 2 stated ZDR exclusions asserted)
    ├── claude-security-workflows.html   Visual companion to claude-security-layers.md · the six-layer ladder + **one workflow diagram per ACTIVE layer** (5 diagrams; layer 6 = your own CI, not Anthropic's) + the gates matrix (guidance-plugin hook lifecycle w/ blind-spot strip · `/security-review` single pass · deep-scan scan→report→patches · Code Review PR track + ultrareview remote-sandbox track w/ the silent-downgrade box · managed-product 4 gates + scan flow) + adjacent-axis cards + the four-way table · 6 structured inline SVGs (literal-hex fills), no JS, prints clean · NOT new content · named -workflows to dodge the .md→.html Jekyll collision · structurally render-verified (6 SVGs, 174 texts, 0 viewBox overflow, 0 clipped nodes, 0 unresolved var() in SVG attrs)
...
Don't, without updating `docs/scope.md` first. The original scope agreed 8 artifacts; the repo currently ships 56 (all post-v1 additions are justified in `scope.md`). New artifacts need a written justification block in `scope.md` and a row in `feature-inventory.md`'s `Used in artifacts` column.

### When the model surface changes

A new model release (e.g., Opus 5.1) is **not** a routine inventory edit — it's a breaking change to every artifact's "current model" reference. **The Models table gained a `Used in artifacts` column in #65** — grep it first, but it is a starting point, not the whole sweep: the tier-pin set is mechanical while the per-model citations need judgment, and advice written against the old model's defaults is invisible to any grep (see [`LESSONS_LEARNED.md`](LESSONS_LEARNED.md) 2026-07-24). Process:
...
5. **Read the model's system card, not just the launch post** ([anthropic.com/system-cards](https://www.anthropic.com/system-cards)). The launch post carries capability and pricing. The system card is the *only* source for the RSP/ASL determination, safeguard **policy** changes (what became newly blocked or unblocked), measured prompt-injection robustness, and the honesty/hallucination profile — governance facts with no docs-page equivalent, which a launch-post-only sweep misses entirely. Land them as `feature-inventory.md` Compliance rows (precedent: Opus 5, 2026-07-24). Carry each number's **test conditions** with it — the cards measure with production safeguards disabled.
6. Update the guards too — `/stale-check`'s model-pin rules encode the *old* pin and will report green on the drift they exist to catch.
7. Update README footer; run `/bump-as-of` for the as-of stamps
8. Blind-review (2 reviewers, factual + decision-utility lenses), then PR — sponsor reviews
